...
public class DtdtsTicketDaoImpl {
    private static final Logger LOGGER = Logger.getLogger(DtdtsTicketDaoImpl.class.getName());
    private static final EntityManagerFactory emf = Persistence.createEntityManagerFactory("db2PU");
    private static final String DATE_FORMAT_PATTERN = "yyyyMMdd";

    /**
     * Alias method to maintain compatibility with controllers calling
     * findReportDataByDateAndTicketRange.
     */
    public List<TicketDto> findReportDataByDateAndTicketRange(Date fromDate, Date toDate,
            String ticketFrom, String ticketTo, String orga) {
        return findReportDataByCriteria(fromDate, toDate, ticketFrom, ticketTo, orga);
    }

    public List<TicketDto> findReportDataByCriteria(Date fromDate, Date toDate, String ticketFrom, String ticketTo,
            String orga) {
        ...
        try (EntityManager em = emf.createEntityManager()) {
            ...
            List<Object[]> queryResult = em.createNativeQuery(sql)
                    ...
                    .setParameter("ticketFrom", fTicket)
                    .setParameter("ticketTo", tTicket)
    ...
    private Integer parseTicket(String ticketStr) {
        return ticketStr != null && !ticketStr.trim().isEmpty() ? Integer.parseInt(ticketStr.trim()) : 0;
    }

    private void logInputFilters(Date fromDate, Date toDate, Integer fDateNum, Integer tDateNum, Integer fTicket,
            Integer tTicket, String orga) {
        LOGGER.info("=== DTDTS REPORT FILTER DEBUG ===");
    ...
    private String buildQuery() {
        return "SELECT " +
                "a.orgaunit AS RC_CODE, f.F0740_ORGAUNIT_DESC AS RC_DESC, a.trtype AS TRANS_TYPE, " +
                ...
                "COALESCE(f.F0740_ORGAUNIT_DESC, '') AS INSTITUTION_NAME " +
                ...
                "LEFT OUTER JOIN CFASLIB.FA0740 f ON a.orgaunit = f.F0740_ORGAUNIT_CODE " +
                "WHERE a.trdat >= :fromDate AND a.trdat <= :toDate " +
                "AND a.ticket >= :ticketFrom AND a.ticket <= :ticketTo " +
                "AND (:orgaCode = '' OR a.orgaunit = :orgaCode) " +
    ...
    private void mapInstitutionName(TicketDto dto, Object[] row) {
        if (row.length > 18 && row[18] != null) {
            dto.setInstitutionName(row[18].toString().trim());
        } else {
    ...
    public JasperPrint generateDtdtsReport(InputStream reportTemplateStream, Map<String, Object> parameters,
        ...
        return generateDtdtsReport(jasperReport, parameters, fromDate, toDate, ticketFrom, ticketTo, orga);
    }
}
